...
class MixedPrecisionModule(Module):
    ...
    def __init__(self, mod: Module, mp_policy: jmp.Policy):
        ...
        self.module = mp_policy.cast_to_param(mod)
    ...
    def __call__(self, *args, **kwargs):
        ...
        * Task 4: It casts the output to the "output" data type.
        """
        old_mod_clone = self.module.copy()
        ...
        self.module.update(casted_mod, in_place=True)

        casted_mod_clone = self.module.copy()
        # task 2
        output = self.module(*casted_args, **casted_kwargs)

        # task 3
        if jax.tree_structure(self.module) != jax.tree_structure(old_mod_clone):
            raise RuntimeError(
                f"The module `{self.module.__class__.__name__}` has its treedef modified during the forward pass. "
        ...
        casted_to_param_mod = jax.tree_map(
            reuse_params_fn, self.module, casted_mod_clone, old_mod_clone
        )

        self.module.update(casted_to_param_mod, in_place=True)
