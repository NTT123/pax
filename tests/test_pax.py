...
def test_type_sequence():
    class Counter(pax.Module):
        ...
        def __init__(self):
            super().__init__()
            self.count = pax.ParameterTree([0])
...
def test_type_dict():
    class Counter(pax.Module):
        ...
        def __init__(self):
            super().__init__()
            self.count = pax.StateTree({"conv1": [1, 2, 3], "conv2": ["a", "b"]})
...
def test_type_dict_dict1():
    class Counter(pax.Module):
        ...
        def __init__(self):
            super().__init__()
            self.count = pax.StateTree(
                {"conv1": {1: [1, 2, 3]}, "conv2": {2: ["a", "b"]}}
            )
...
def test_state_of_param():
    class M1(pax.Module):
        def __init__(self):
            super().__init__()
            self.p1 = pax.Parameter(jnp.array(0.0, dtype=jnp.float32))
